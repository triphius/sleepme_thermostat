...
def test_build_device_info_shape():
    ...
    # (callers pass entry.title), so "SleepMe Dock Pro - Ramon" goes in unchanged.
    out = build_device_info("dev-1", "SleepMe Dock Pro - Ramon", info)
    assert out["identifiers"] == {(DOMAIN, "dev-1")}
    assert out["name"] == "SleepMe Dock Pro - Ramon"
...
def test_build_device_info_handles_missing_keys():
    ...
    out = build_device_info("dev-2", "SleepMe Dock Pro - Chiva", {})
